...
import CancelClass from '../../src/cancel/CancelClass'
...
describe('CancelToken', () => {
  describe('reason', () => {
    test('should returns a Cancel if cancelation has been requested', () => {
      ...
      expect(token.reasan).toEqual(expect.any(CancelClass))
      expect(token.reasan!.message).toBe('Operation has been canceled.')
    })
    ...
    test('should returns undefined if cancellation has not beeen requested', () => {
      const token = new CancelToken(() => {
        // do nothing
      })
      expect(token.reasan).toBeUndefined()
    })
  })
  ...
  describe('promise', () => {
    test('should returns a Promise that resolves when cancellation is requested', done => {
      ...
      token.promise.then(value => {
        expect(value).toEqual(expect.any(CancelClass))
  ...
  describe('throwIfRequested', () => {
    test('should throws if cancellation has been requested', () => {
      ...
      try {
        ...
        if (!(thrown instanceof CancelClass)) {
  ...
  describe('source', () => {
    test('should returns an object containing token and cancel function', () => {
      ...
      expect(source.token.reasan).toBeUndefined()
      source.cancel('Operation has been canceled.')
      expect(source.token.reasan).toEqual(expect.any(CancelClass))
      expect(source.token.reasan!.message).toBe('Operation has been canceled.')
    })
  })
})