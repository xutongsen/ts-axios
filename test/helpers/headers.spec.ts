import { processHeader, parseHeaders, flattenHeaders } from '../../src/helpers/headers'
...
describe('helpers: header', () => {
  ...
  describe('processHeaders', () => {
    ...
    test('should set Content-Type if not set and data is PlainObject', () => {
      const headers: any = {}
      processHeader(headers, { a: 1 })
      expect(headers['Content-Type']).toBe('application/json;charset=utf-8')
    })

    test('should set not Content-Type if not set and data is not PlainObject', () => {
      const headers: any = {}
      processHeader(headers, new URLSearchParams('a=b'))
      expect(headers['Content-Type']).toBeUndefined()
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(processHeader(undefined, {})).toBeUndefined()
      expect(processHeader(null, {})).toBeNull()
    })
  })
  ...
  describe('flattenHeaders', () => {
    ...
    test('should flatten the headers without cmmmon headers', () => {
      const headers = {
        Accept: 'application/json',
        get: {
          'X-GET-HEADER': 'getHeaderValue'
